...
class TiendaActualizarSerializer(Serializer):
    ...
    #         if value>0:
    #             verifica_id= SUCURSALES.objects.filter(SUC_ID=value).exists()
    #             if not verifica_id:
    #                 raise serializers.ValidationError("No existe tal Sucursal")
    #             # else:
    #             #     raise serializers.ValidationError("SI existe tal Sucursal")
    #         else:
    ...
    def update(self, instance, data):
        ...
        print("DATA:  ",data)
        instanciamiento=SUCURSALES()
        instanciamiento.SUC_ID=data.get('tie_suc_id', instance.tie_suc_id)


        # instance.tie_nombre = str(data.get('tie_nombre', instance.tie_nombre)).upper()
        
        # instance.tie_suc_id= instanciamiento
        # tienda_suc_id
        # instance.tie_suc_id=str(instanciamiento if instanciamiento else instance.get('tie_suc_id'))
        # instance.tie_suc_id=data.get('tie_suc_id',instance.tie_suc_id)
        # instance.sucu_tie_id= data.get('tie_suc_id', instance.sucu_tie_id)
