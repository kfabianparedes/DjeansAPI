...
class GuiaDeRemisionRegistrarSerializer(Serializer):
    ...
    def validate_guia_serie(self, value):
        if validarNumeroSerie(value):
            ...
            raise serializers.ValidationError("La serie de la guía tiene que empezar con B o F "
    ...
    def validate_guia_numero(self, value):
        if validarEsNumerico(value):
            if 4 <= len(str.strip(value)) <= 8:
                return value
            raise serializers.ValidationError("La cantidad de caracteres del número de guía debe tener entre 4 "
                                              "y 8 dígitos.")
        else:
